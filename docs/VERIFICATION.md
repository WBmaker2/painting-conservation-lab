...
|---|---|---|---|---|
| 모바일(Moto G4 시뮬) | 71 | 4.7s | 0ms | 0 |
| 데스크톱 | 99 | 0.8s | 0ms | ~0 |

- `npm run build`: 12모듈, dist 56K (js 38K/gzip 12.9K). 앱 코드는 전체 바이트의 약 7%.
- 병목: Noto Serif KR woff2 7종 ≈260KB가 로드 시간의 거의 전부 (구글 폰트 한글 subset 분할).
- 시도: 폰트 CSS 비동기화(`media=print`+`onload`, 커밋 0a8cc92) → lab 수치 변화 없음(71→69, 오차 범위). woff2 페이로드 자체가 지배적이라 링크 방식만으로는 안 움직임.
- 판단: lab 4.7s는 4x 스로틀 시뮬레이션 수치라 실기기보다 가혹하게 나옴. 진짜로 줄이려면 웹폰트 제거(시스템 serif 폴백) 또는 600 단일 웨이트가 필요 — 시각 디자인 변경이라 임의로 안 하고 선택지로 남김.
